...
def test_assessment_request_model():
    ...
    components = [
        SystemComponent(
            id="frontend-1", type=ComponentType.FRONTEND, label="React App"
        ),
        SystemComponent(id="backend-1", type=ComponentType.BACKEND, label="API Server"),
    ]

    connections = [
        Connection(
            id="conn-1", source="frontend-1", target="backend-1", label="HTTP requests"
        )
    ]
...
def test_score_breakdown_validation():
    ...
    # Test valid scores
    ScoreBreakdown(scalability=100, reliability=0, security=50, maintainability=25)
...
def test_assessment_response_model():
    ...
    scores = ScoreBreakdown(
        scalability=85, reliability=90, security=75, maintainability=80
    )
