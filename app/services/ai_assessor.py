...
from app.models.response_models import AssessmentResponse, ScoreBreakdown, ValidationFeedback
...
class AIAssessorService:
    ...
    def _transform_ai_response(self, ai_result: Dict[str, Any]) -> AssessmentResponse:
        ...
        feedback = [
            ValidationFeedback(**fb) for fb in ai_result.get("feedback", [])
        ]
    ...
    def _fallback_assessment(self, request: AssessmentRequest, error: str) -> AssessmentResponse:
        ...
        if has_database: base_score += 10
        if has_load_balancer: base_score += 15
